...
public class WatchedMoviesController {
    ...
    @GetMapping
    public ResponseEntity<?> getWatchedMovies(@AuthenticationPrincipal Jwt jwt, @RequestParam(defaultValue = "0") int page,
                                              @RequestParam(defaultValue = "10") int size) {
        var userId = jwt.getClaimAsString("sub");
        var pageable = PageRequest.of(page, size);
        var movies = watchedService.getWatchedMoviesByUserId(userId, pageable);
